...
class FrmDosen:
    ...
    def aturKomponen(self):
        ...
        Cbo_jk.current()
        # Button
        self.btnCari = Button(mainFrame, text='Cari', command=self.onCari, width=10, fg='blue', bg='red')
        self.btnCari.grid(row=0, column=3, padx=5, pady=5)
        self.btnSimpan = Button(mainFrame, text='Simpan', command=self.onSimpan, width=10,fg='yellow', bg='blue')
        self.btnSimpan.grid(row=1, column=3, padx=5, pady=5)
        self.btnClear = Button(mainFrame, text='Clear', command=self.onClear, width=10,fg='black', bg='lightblue')
        self.btnClear.grid(row=0, column=4, padx=5, pady=5)
        self.btnHapus = Button(mainFrame, text='Hapus', command=self.onDelete, width=10,fg='gold', bg='green')
        self.btnHapus.grid(row=1, column=4, padx=5, pady=5)
        # define columns
        columns = ('id_dosen','nidn','nama','jk')
        self.tree = ttk.Treeview(mainFrame, columns=columns, show='headings')
        # define headings
        self.tree.heading('id_dosen', text='ID_DOSEN')
        self.tree.column('id_dosen', width="100")
        self.tree.heading('nidn', text='NIDN')
        self.tree.column('nidn', width="100")
    ...
    def onReload(self, event=None):
        ...
        for i, d in enumerate(parsed_data):
            self.tree.insert("", i, text="Item {}".format(i), values=(d["id_dosen"],d["nidn"],d["nama"],d["jk"]))
